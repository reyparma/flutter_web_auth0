# flutter_web_auth0

Flutter Web Authentication Using Auth0, BloC and Go_Router

There are a lot of Flutter authentication examples using BloC and Go_Router with Firebase or Google IAM.  This Flutter web application works with Auth0 for authentication, BLoC for state management and Go_Router for navigation.  

Read the article: https://medium.com/@reyparma_53717/flutter-web-authentication-using-auth0-bloc-and-go-router-62d7ebc91e7a

<img width="600" alt="app_login" src="https://github.com/reyparma/flutter_web_auth0/assets/5339530/b4c1b878-7cb2-44d3-87cc-0ace47edb85d">
<img width="600" alt="app_popupx" src="https://github.com/reyparma/flutter_web_auth0/assets/5339530/efac7db1-a5ec-4864-8d85-e25df9ffdc66">
<img width="600" alt="app_profile" src="https://github.com/reyparma/flutter_web_auth0/assets/5339530/df042b62-a524-4a60-a19e-1aba76d8b69a">

